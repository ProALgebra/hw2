...
typedef struct {
  uint32_t next_offset;   /* offset of the next instruction in the stream */
  uint32_t size;          /* size of the current instruction in bytes */
  uint32_t targets[2];    /* explicit control-flow targets (if any) */
  uint8_t target_count;   /* number of entries in targets */
  uint8_t breaks_flow;    /* non-zero if there is no fallthrough */
  uint8_t group;          /* high nibble of opcode */
  uint8_t opcode;         /* low nibble of opcode */
} instr_info;
...
static inline char *get_public_name(bytefile *f, uint32_t i)
{
  return get_string(f, f->public_ptr[i * 2]);
}

static inline uint32_t get_public_offset(bytefile *f, uint32_t i)
{
  return f->public_ptr[i * 2 + 1];
}

static inline bytefile *read_file(char *fname)
{
